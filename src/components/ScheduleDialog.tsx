...
const blank: ScheduleInput = { judul: "", kategori: "Tugas", prioritas: "Medium", status: "Belum Selesai", deadline: "" };
...
export function ScheduleDialog({ schedule, open, saving, onClose, onSave }: { schedule: Schedule | null; open: boolean; saving: boolean; onClose(): void; onSave(input: ScheduleInput): Promise<void> }) {
  ...
  useEffect(() => { if (open) { setForm(schedule ? { judul: schedule.judul, kategori: schedule.kategori || "Tugas", prioritas: schedule.prioritas, status: schedule.status, deadline: localDateTime(schedule.deadline) } : blank); setError(""); setTimeout(() => firstInput.current?.focus(), 0); } }, [open, schedule]);
  if (!open) return null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-md" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}>
      <div role="dialog" aria-modal="true" aria-labelledby="dialog-title" className="glass-card w-full max-w-md p-6 sm:p-8 shadow-2xl animate-fade-in-up border border-white/20 dark:border-white/10">
        <div className="flex items-center justify-between">
          <h2 id="dialog-title" className="text-xl font-bold">{schedule ? "Edit jadwal" : "Tambah jadwal"}</h2>
        ...
        <form onSubmit={(e) => { void submit(e); }} className="mt-5 space-y-4">
          <div>
            ...
            <input ref={firstInput} id="title" required maxLength={200} value={form.judul} onChange={(event) => setForm({ ...form, judul: event.target.value })} className="input-glass w-full" placeholder="Masukkan judul tugas" />
          </div>
          <div>
          ...
          <div>
            ...
            <input id="deadline" type="datetime-local" required value={form.deadline} onChange={(event) => setForm({ ...form, deadline: event.target.value })} className="input-glass w-full" />
          </div>
